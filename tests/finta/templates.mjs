...
// Le due catene del Tiro (27/9): chi dà la soglia (incantesimo, Sfere, Ambiti) e chi dà dadi.
context.tiro.pillsSoglia = context.tiro.pills.filter((pill) => ["spell", "sphere", "scope"].includes(pill.kind));
context.tiro.pillsDadi = context.tiro.pills.filter((pill) => !["spell", "sphere", "scope"].includes(pill.kind));
const html = stat(context);
// Le tre colonne del Tiro: le teste delle catene, la pillola dell'incantesimo nella colonna della soglia, il premio +3.
for (const marker of ['wod5e-mage-tiro-colonna-testa">WOD5E_MAGE.Tiro.CatenaDadi', 'wod5e-mage-tiro-catena wod5e-mage-tiro-catena-dadi', 'wod5e-mage-tiro-colonna-testa">WOD5E_MAGE.Tiro.CatenaSoglia', 'wod5e-mage-tiro-catena wod5e-mage-tiro-catena-soglia', "wod5e-mage-tiro-centro", "<b>+3</b>"]) {
  assert.ok(html.includes(marker), `manca ${marker}`);
}
assert.ok(html.indexOf('wod5e-mage-pillola tipo-spell') > html.indexOf("wod5e-mage-tiro-catena-soglia"), "l'incantesimo sta nella catena della soglia");
assert.ok(html.indexOf('wod5e-mage-pillola tipo-arete') < html.indexOf("wod5e-mage-tiro-centro"), "l'Areté sta nella catena dei dadi");
