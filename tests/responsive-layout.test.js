...
// Catalogo completo e Aggiungi (la finestra con le pastiglie delle Sfere); la riga del potere a colonne fisse (27/9:
// sigillo della Sfera, grado, nome, le pastiglie del tipo, Amalgama, usi, costo, Usa, la freccia che è un tasto), le quattro
// parti che si aprono (Grado, Prerequisiti, Effetto attivo, Effetto passivo), Modifica, Per tutti e Togli. Niente tendina sotto la Sfera, niente sezioni.
assert.match(magickTemplate, /wod5e-mage-potere-riga\{\{#if p\.editing\}\} aperta modifica\{\{\/if\}\}" data-row="\{\{p\.id\}\}"[\s\S]*wod5e-mage-potere-testa[\s\S]*wod5e-mage-potere-sfera" src="\{\{p\.sphereIcon\}\}"[\s\S]*wod5e-mage-potere-pallino[\s\S]*wod5e-mage-potere-nome" data-action="potereApri"[\s\S]*wod5e-mage-potere-tipi[\s\S]*wod5e-mage-potere-tipo attivo[\s\S]*wod5e-mage-potere-tipo passivo[\s\S]*wod5e-mage-potere-amalgama[\s\S]*wod5e-mage-potere-usi[\s\S]*wod5e-mage-potere-costo[\s\S]*<button type="button" class="wod5e-mage-potere-usa\{\{#unless p\.usa\.ok\}\} spento\{\{\/unless\}\}\{\{#unless p\.tipi\.attivo\}\} mostra\{\{\/unless\}\}" data-action="potereUsa" data-row="\{\{p\.id\}\}"[\s\S]*Poteri\.Mostra[\s\S]*<button type="button" class="wod5e-mage-potere-chevron" data-action="potereApri"[\s\S]*wod5e-mage-potere-spiega[\s\S]*name="flags\.wod5e-mage\.poteri\.\{\{p\.id\}\}\.name"[\s\S]*name="flags\.wod5e-mage\.poteri\.\{\{p\.id\}\}\.dot" data-dtype="Number"[\s\S]*\.type"[\s\S]*\.cost"[\s\S]*\.prerequisitiTesto"[\s\S]*\.attivo"[\s\S]*\.passivo"[\s\S]*\.amalgam"[\s\S]*\.amalgamText"[\s\S]*\.flavor"[\s\S]*wod5e-mage-potere-parti[\s\S]*wod5e-mage-potere-blocco prerequisiti[\s\S]*Poteri\.Prerequisiti[\s\S]*wod5e-mage-potere-blocco attivo[\s\S]*Poteri\.EffettoAttivo[\s\S]*\{\{#each p\.parti\.attivo\.voci as \|v\|\}\}[\s\S]*<b>\{\{v\.chiave\}\}:<\/b>[\s\S]*wod5e-mage-potere-blocco passivo[\s\S]*Poteri\.EffettoPassivo[\s\S]*wod5e-mage-potere-fondo[\s\S]*data-action="potereModifica" data-row="\{\{p\.id\}\}"[\s\S]*data-action="potereModificaBase" data-row="\{\{p\.id\}\}"[\s\S]*data-action="potereTogli" data-row="\{\{p\.id\}\}"/, "la riga del potere a colonne (27/9), le quattro parti aperte, Modifica, Per tutti, Togli");
...
assert.match(css, /\.wod5e-mage-potere-testa\s*\{[^}]*display: grid;[^}]*grid-template-columns: 22px 20px minmax\(0, 1fr\) 140px 22px 84px 58px 64px 18px;/s);
