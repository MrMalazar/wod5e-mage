...
// I poteri conosciuti: la testa della Sfera con Aggiungi (dal 24/9 sera apre la finestra del catalogo)
// e la penna per scrivere a mano; la riga del potere a colonne (pallino, nome, tipo, Amalgama, costo,
// freccia), il testo che si apre, Modifica e Togli. La tendina del catalogo sotto la Sfera non c'è più.
assert.match(magickTemplate, /wod5e-mage-riga wod5e-mage-poteri-testa"[\s\S]*wod5e-mage-poteri-eco[\s\S]*wod5e-mage-poteri-conto[\s\S]*wod5e-mage-poteri-aggiungi" data-action="potereCatalogo" data-sphere="\{\{sez\.id\}\}"[\s\S]*wod5e-mage-poteri-penna" data-action="potereNuovo" data-sphere="\{\{sez\.id\}\}"/);
assert.doesNotMatch(magickTemplate, /wod5e-mage-cassetto-catalogo|potereDaCatalogo|cassettoToggle" title="\{\{localize 'WOD5E_MAGE\.Poteri\.AggiungiHint'/);
