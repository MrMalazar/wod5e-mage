...
// Nove righe: i sette Ambiti, con la Potenza sdoppiata in Effetto e Danni
// e la Durata in gioco e narrativa.
assert.equal(table.rows.length, 9);
assert.deepEqual(table.rows.map((row) => row.id), ["potency", "potencyDamage", "duration", "durationNarrative", "area", "targets", "conditions", "range", "precision"]);
assert.equal(table.rows[1].scope, "potency");
assert.equal(table.rows[3].scope, "duration");
...
assert.equal(table.rows[1].cells[0].arete, true);
assert.equal(table.rows[1].cells[0].hideLabel, true);
assert.equal(table.rows[1].cells[1].hideLabel, false);
assert.equal(table.rows[1].cells[0].glyph, undefined);
assert.equal(table.rows[5].cells[0].faIcon, "fa-solid fa-user");
assert.equal(table.rows[6].cells[0].faIcon, "fa-solid fa-list-check");
assert.equal(table.rows[3].cells[6].faIcon, "fa-solid fa-infinity");
assert.equal(table.rows[4].cells[0].faIcon, "fa-solid fa-door-open");
...
assert.equal(table.rows[3].cells[0].icon, "");
...
// Le 63 celle della tavola hanno una voce in tutte e due le lingue.
...
assert.deepEqual(table.rows.map((row) => row.layout), ["text", "symbol-number", "symbol-number", "symbol-text", "symbol-text", "symbol-number", "symbol-number", "text", "text"]);
assert.equal(table.rows[1].cells[0].number, false);
assert.equal(table.rows[1].cells[1].number, true);
assert.equal(table.rows[8].small, true);
assert.equal(table.rows[8].cells[0].text, true);
