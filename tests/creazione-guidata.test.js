...
// I quattordici passi dell'ordine del 5/9 (il mock del 23/9), con la Sottofamiglia a parte (25/9 sera).
assert.deepEqual([...PASSI], ["credo", "famiglia", "sottofamiglia", "bussola", "tipo", "concetto", "sfere", "strumenti", "arete", "attributi", "abilita", "vantaggi", "ancore", "controllo"]);
...
assert.equal(g1.passi.length, 14);
...
// Il passo 2: le Famiglie per fazione, quelle col Credo davanti; la via è il passo 3 (25/9 sera).
...
assert.deepEqual(g2.corpo.dotted, []);
...
assert.deepEqual([g2b.corpo.family.id, g2b.corpo.family.subKind, g2b.corpo.family.sphere.label, g2b.corpo.family.conVie], ["verbena", "Circolo", "Vita", true]);
assert.deepEqual(g2b.corpo.dotted.map((s) => s.id), ["life"], "al passo 2 solo la Sfera di famiglia");
...
assert.deepEqual(g2b.avanti, { n: 3, label: "Sottofamiglia" });
assert.equal(prepareGuidata(attore({ flags: { lineage: { famiglia: "verbena" } } }), { passo: 2, localize }).passi[1].fatto, true, "la Famiglia è fatta con la Famiglia sola (25/9 sera)");
assert.equal(prepareGuidata(attore({ flags: { lineage: { famiglia: "ngoma" } } }), { passo: 2, localize }).corpo.family.craft, true);
assert.equal(prepareGuidata(attore({ flags: { lineage: { famiglia: "ngoma" } } }), { passo: 2, localize }).corpo.pratica, praticaDi("ngoma").forma, "la Craft dice la sua pratica qui");

// Il passo 3 (25/9 sera): la Sottofamiglia, le quattro vie della Famiglia scelta; una Craft non ha vie.
const g3s = prepareGuidata(verbena, { passo: 3, localize });
assert.equal(g3s.passo.id, "sottofamiglia");
assert.equal(g3s.corpo.sottofamiglie.length, 4);
const streghe = g3s.corpo.sottofamiglie.find((s) => s.id === "streghe");
...
assert.deepEqual([g3s.corpo.sub.label, g3s.corpo.sub.credoLabel, g3s.corpo.pratica], ["Streghe della Siepe", "Tutto è Sacro", "Sciamanesimo"]);
assert.deepEqual(g3s.corpo.dotted.map((s) => s.id), ["life", "spirit"]);
assert.equal(g3s.nota, "Sulla scheda: Appartenenza Verbena · Streghe della Siepe, con le sue Sfere a 1");
assert.deepEqual([g3s.passi[2].fatto, g3s.avanti], [true, { n: 4, label: "Bussola" }]);
const senzaVia = prepareGuidata(attore({ flags: { lineage: { famiglia: "verbena" } } }), { passo: 3, localize });
assert.deepEqual([senzaVia.passi[1].fatto, senzaVia.passi[2].fatto, senzaVia.corpo.sub, senzaVia.nota], [true, false, null, "Sulla scheda: niente ancora"], "senza la via il passo 3 non è fatto");
const craft = prepareGuidata(attore({ flags: { lineage: { famiglia: "ngoma" } } }), { passo: 3, localize });
assert.deepEqual([craft.passi[2].fatto, craft.corpo.family.craft, craft.corpo.family.conVie, craft.corpo.sottofamiglie], [true, true, false, []], "una Craft non ha vie: il passo è fatto");
assert.equal(prepareGuidata(vuoto, { passo: 3, localize }).corpo.family, null);
...
const g3 = prepareGuidata(attore({ flags: { focus: { credo: "sacro" }, convinzioni: { r1: { group: "morte", text: "Nessuno decide", serve: "rifiuti", cross: "uccidi" } } }, headers: { ambition: "Trovare mia sorella" } }), { passo: 4, localize, cataloghi });
...
assert.equal(g3.passi[3].fatto, false, "manca il Desiderio");
assert.equal(prepareGuidata(attore({ flags: { convinzioni: { r1: { text: "x" } } }, headers: { ambition: "a", desire: "b" } }), { passo: 4, localize }).passi[3].fatto, true);
...
// Il passo 6: il Concetto, i gruppi della Sfida e i premi.
const g5 = prepareGuidata(attore({ flags: { conceptChallenge: { description: "a", mask: "b", routine: "c", livelihood: "d", spark: "e", cracks: "f", outlet: "g" } }, headers: { concept: "Infermiera" } }), { passo: 6, localize, cataloghi: { concetto: [{ uuid: "k1", name: "Infermiera di corsia", text: "Infermiera di corsia", group: "Concetti" }] } });
...
const g6 = prepareGuidata(attore({ flags: conDomini }), { passo: 7, localize, lang: "it" });
...
assert.equal(g6.passi[6].fatto, false);
// Col potere di Spirito il passo è fatto; senza Credo né Famiglia il passo lo dice; una Craft senza vie ha due Domini.
const g6b = prepareGuidata(attore({ flags: { ...conDomini, poteri: { ...conDomini.poteri, c: { sphere: "spirit", catalogId: "coro", name: "Coro" } } } }), { passo: 7, localize, lang: "it" });
assert.deepEqual([g6b.corpo.conto.poteri.text, g6b.passi[6].fatto], ["3/3", true]);
const g6c = prepareGuidata(attore({ flags: { arete: { value: 1 } } }), { passo: 7, localize, lang: "it" });
assert.deepEqual([g6c.corpo.senzaAppartenenza, g6c.corpo.conto.domini.text, g6c.corpo.spheres.every((s) => !s.ruolo)], [true, "0/3", true]);
const g6d = prepareGuidata(attore({ flags: { arete: { value: 1 }, lineage: { famiglia: "ngoma" }, focus: { credo: "arte", credoFamily: "matter" }, selectedSpheres: { prime: true, matter: true }, poteri: { a: { sphere: "prime", catalogId: "faro", name: "Faro" }, b: { sphere: "matter", catalogId: "bottino", name: "Bottino" } } } }), { passo: 7, localize, lang: "it" });
assert.deepEqual([g6d.corpo.conto.domini.text, g6d.corpo.conto.poteri.text, g6d.passi[6].fatto], ["2/2", "2/2", true], "una Craft senza vie: due Domini");
// Il grado Discepolo chiede due poteri in più.
const g6e = prepareGuidata(attore({ flags: { ...conDomini, arete: { value: 3 }, creazione: { grado: "discepolo" } } }), { passo: 7, localize, lang: "it" });
assert.deepEqual([g6e.corpo.conto.poteri.text, g6e.corpo.extra], ["2/5", 2]);
// Il passo 8: uno Strumento per Sfera aperta e quello di Percepire; il consiglio del Credo per Sfera.
const g7 = prepareGuidata(attore({ flags: { focus: { credo: "vivo", practiceForm: "magick", sphereInstruments: { life: { tool: "herbs", name: "la salvia del balcone" } } }, spheres: { life: 1, forces: 2 }, selectedSpheres: { life: true, forces: true } } }), { passo: 8, localize, lang: "it" });
...
assert.equal(g7.passi[7].fatto, false);

// Il passo 9: l'Areté del grado, Quintessenza e Paradosso.
const g8 = prepareGuidata(attore({ flags: { arete: { value: 2 }, magickBalance: { quintessence: 3, paradox: 0 } } }), { passo: 9, localize });
...
assert.equal(prepareGuidata(attore({ flags: { arete: { value: 3 }, creazione: { grado: "discepolo" } } }), { passo: 9, localize }).corpo.ok, true);
assert.equal(g8.nota, "Sulla scheda: Areté 2");

// Il passo 10: i tre gruppi, la forma, le statistiche di base (05_100).
const g9 = prepareGuidata(vuoto, { passo: 10, localize });
...
// Il passo 11: le quattordici Abilità, il tetto, il conto (19 pallini).
const g10 = prepareGuidata(vuoto, { passo: 11, localize });
...
// Il passo 12: Background, Pregi e Difetti coi conti (24/9: 5 Pregi più 4 Background, 2 Difetti obbligatori).
const g11 = prepareGuidata(attore({ items: [feature("Alleati", "background", 3), feature("Ambidestro", "merit", 4), feature("Contatti", "background", 2), feature("Dipendenza", "flaw", 2)] }), { passo: 12, localize });
...
const g11b = prepareGuidata(attore({ items: [feature("Alleati", "background", 3), feature("Ambidestro", "merit", 4), feature("Contatti", "background", 2), feature("Dipendenza", "flaw", 3)] }), { passo: 12, localize });
...
assert.equal(g11.passi[11].fatto, true);

// Il passo 13: le Ancore a sette righe (25/9), con le dodici montate del catalogo come proposte.
const g12 = prepareGuidata(attore({ flags: { convinzioni: { c1: { text: "Nessuno decide chi vive e chi muore.", group: "morte" } }, ancore: { x1: { name: "Giacomo", role: "chi ti serve ogni giorno", job: "barista", age: "27", gives: "nessuna domanda", conviction: "c1", unknown: "cosa sei", bites: "può vederti" }, x2: { role: "partner" } } } }), { passo: 13, localize, cataloghi: { ancora: [{ uuid: "n1", name: "La madre che dimentica", text: "La madre che dimentica", role: "genitore", gives: "il prima: chi eri", unknown: "chi sei diventato", bites: "può non riconoscerti più", group: "Ancore" }] } });
...
assert.equal(g12.passi[12].fatto, true);
assert.deepEqual(g12.avanti, { n: 14, label: "Controllo" });

// Il passo 14 e i passi fatti: un personaggio intero è tutto verde.
const intero = attore({
  flags: {
...
const g13v = prepareGuidata(vuoto, { passo: 14, localize });
...
assert.equal(prepareGuidata(vuoto, { passo: 99, localize }).passo.n, 14, "un passo fuori scala si ferma all'ultimo");

// La finestra e i template: quattordici partial, il perché, i tasti in testata, l'apertura sul Mago nuovo.
